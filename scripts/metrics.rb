...
# mapping - the replacement stratedgy
# n       - n column mesh
def tfo_delta_variance(tg,mapping,n)
  avg = avg_traffic_flow_occupancy(tg,{},n)
  avg_dif = normalized_avg_tfo_delta(tg,mapping,n)
  Math.sqrt(tg[1].inject(0.0){ |sum,edge|
              sum + (tfo_delta(tg,mapping,edge[0],edge[1],n)/avg - avg_dif)**2
            }/tg[1].size)
end
...
def normalized_avg_tfo_delta(tg,mapping,n)
  tg[1].inject(0.0){ |sum,edge|
    sum + tfo_delta(tg,mapping,edge[0],edge[1],n)
  }/(avg_traffic_flow_occupancy(tg,{},n)*tg[1].size)
end
...
# n       - n column mesh
def tfo_delta(tg,mapping,i,j,n)
  (traffic_flow_occupancy(tg,{},i,j,n) - traffic_flow_occupancy(tg,mapping,i,j,n)).abs
end
